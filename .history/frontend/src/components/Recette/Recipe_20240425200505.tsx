...
interface IRecipeProps {
    ...
    openRecipeToEdit:(recipe:IRecipe)=>void
}
interface IData {
    isDone: boolean;
...
export const Recipe=({recipe , deleteRecipe , openRecipeToEdit , onChangeRecipe}:IRecipeProps)=>{
    ...
    return(
        <div key={`recipe : ${recipe._id}`} className={`rounded-3xl ${isDone ? "bg-green-800" : "bg-gray-100" } gap-5 flex flex-col pb-5 pt-5 pr-10 pl-10 text-center`} > 
                       <h4 className="text-2xl capitalize" onClick={() => onChangeRecipe({ ...recipe, status: newStatus }, recipe._id)}>{recipe.title}</h4>
